...
graph TD
    ...
    DEB["Debezium<br/>(WAL 읽기)"]
    KAFKA["Kafka auction-events 토픽"]
...
graph TD
    TOPIC["bid-events 토픽"]
    KS["Kafka Streams"]
    SS["State Store (RocksDB)<br/>key: auctionId<br/>value: {currentPrice, currentWinnerId, bidCount}"]
    API["GET /auctions/{id}"]
...
graph LR
    ...
    NS2["Notification Service (Instance 2)"]
    NS3["Notification Service (Instance 3)"]
    REDIS[("Redis<br/>세션 공유")]
