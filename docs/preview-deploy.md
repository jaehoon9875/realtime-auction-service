...
smoke test까지 실행한 뒤 자동으로 `:latest` 이미지로 복구한다.
...
│  ③ Gateway 헬스체크 대기                             │
│  ④ smoke test 실행                                  │
│  ⑤ 복구: 변경 서비스 kubectl set image (:latest)     │
│     (성공/실패 무관하게 항상 실행)                     │
└─────────────────────────────────────────────────────┘
```
