...
├── check-cluster-state.sh  # 테스트 전후 클러스터 상태 확인(읽기 전용)
...
./perf/check-cluster-state.sh    # 테스트 전후 클러스터 상태 확인
./perf/run-perf.sh               # baseline.env 기본값으로 측정
./perf/run-perf.sh stop          # Job/ConfigMap 정리
```

상세 절차·실행 시 주의사항은 [docs/performance/README.md](../docs/performance/README.md) 참고.
